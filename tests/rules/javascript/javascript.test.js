...
test('javascript', async () => {
  ...
  expect(results[0].messages).toEqual(
    expect.arrayContaining([
      ...
      expect.objectContaining({ ruleId: 'no-constant-condition' }),
      expect.objectContaining({ ruleId: 'arrow-body-style' }),
      expect.objectContaining({ ruleId: 'constructor-super' }),
      ...
      expect.objectContaining({ ruleId: 'no-useless-constructor' }),
      expect.objectContaining({ ruleId: 'no-useless-rename' }),
      expect.objectContaining({ ruleId: 'no-var' }),
      expect.objectContaining({ ruleId: 'object-shorthand' }),
      expect.objectContaining({ ruleId: 'prefer-arrow-callback' }),
      expect.objectContaining({ ruleId: 'no-shadow' }),
      expect.objectContaining({ ruleId: 'no-use-before-define' }),
      expect.objectContaining({
        ruleId: 'camelcase',
      ...
      expect.objectContaining({ ruleId: 'no-array-constructor' }),
      expect.objectContaining({ ruleId: 'max-classes-per-file' }),
      expect.objectContaining({ ruleId: 'no-param-reassign' }),
    ]),
  );
});
